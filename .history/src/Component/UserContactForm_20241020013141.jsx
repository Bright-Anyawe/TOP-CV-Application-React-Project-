...
export function UserContactForm({
  ...
  function handleUserName(e) {
    localStorage.setItem("userContact", JSON.stringify(uer));
  ...
  function handleInputEmpty() {
    setUserContact(
      (prevState = { ...prevState, taskTitle: "", email: "", mobileNumber: "" })
    );
  }
  ...
  function handleFormValidation(e) {
    ...
    } else if (!userContact.personName.match(/^[A-Za-z][A-Za-z0-9_ ]{3,29}$/)) {
      return setError(
    ...
    if (userContact.mobileNumber && userContact.mobileNumber.length !== 10) {
      setPhoneNumberError(
        `You entered ${userContact.mobileNumber.length} digits, the value should be 10 digits. try again`
      );
