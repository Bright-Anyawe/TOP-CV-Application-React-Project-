...
export function useUserContactForm() {
  ...
  const [valueDisplay, setValueDisplay] = useState(false);
   ...
   useEffect(() => {
            ...
            Contact = JSON.parse(localStorage.getItem('userContact')) 
            console.log(userContact)
    // This function will run when the component is first loaded
  if(Object.keys(userContact).length !== 0 && userContact.constructor !== Object
    ) {
    ...
    return handleUserContactDisplayForEdit()
    alert('Page has been refreshed or loaded for the first time');
  }, []);
