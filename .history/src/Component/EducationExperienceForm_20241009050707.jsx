...
export function useEducationExperienceForm() {
  ...
  const [valueDisplay, setValueDisplay] = useState(false);
    ...
    window.onload = () => {
      let educationExperience = JSON.parse(localStorage.getItem("educationExperience"));
      console.log(educationExperience);
      if (Object.keys(educationExperience).length !== 0) {
        setIsSubmit(true);
...
updateStudiesDate    );
  ...
  function handleEducationExperience(
    ...
    localStorage.setItem("educationExperience", JSON.stringify(personContact));
